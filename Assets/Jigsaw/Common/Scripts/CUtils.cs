...
public class CUtils
{
    ...
    public static void ShowChildInScrollView(ScrollRect scrollRect, int index)
    {
        if (scrollRect.content.childCount == 0) return;
        index = Mathf.Clamp(index, 0, scrollRect.content.childCount - 1);

        if (scrollRect.vertical)
        {
            var child = scrollRect.content.GetChild(index);
            float localY = child.localPosition.y;
            float scrollHeight = scrollRect.GetComponent<RectTransform>().rect.height;
            float contentY = -localY - scrollHeight / 2;
            float maxY = scrollRect.content.rect.height - scrollHeight;
            contentY = Mathf.Max(Mathf.Min(contentY, maxY), 0);
            scrollRect.content.localPosition = new Vector2(scrollRect.content.localPosition.x, contentY);
        }
        else
        {
            var child = scrollRect.content.GetChild(index);
            ...
            float contentX = -(localX - scrollWidth / 2);
            float minX = -(scrollRect.content.rect.width - scrollWidth);
            contentX = Mathf.Min(Mathf.Max(minX, contentX), 0);

            scrollRect.content.localPosition = new Vector2(contentX, scrollRect.content.localPosition.y);
        }
    }
}
